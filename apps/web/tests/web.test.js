...
describe('web app', () => {
    ...
    it('renders a deep-linked request and keeps that request selected', async () => {
        render(_jsxs(MemoryRouter, { initialEntries: ['/requests/req-2'], children: [_jsx(App, {}), _jsx(LocationDisplay, {})] }));
        const detailPane = await screen.findByRole('main');
        expect(within(detailPane).getByRole('heading', { name: 'Review database migration' })).toBeInTheDocument();
    ...
    it('updates the URL when selecting a different request', async () => {
        render(_jsxs(MemoryRouter, { initialEntries: ['/'], children: [_jsx(App, {}), _jsx(LocationDisplay, {})] }));
