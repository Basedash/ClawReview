import type {
  RequestReviewState,
  RequestStatus,
  ResumeStatus,
  ReviewAction,
} from '@clawreview/shared';

import { DomainConflictError } from '../errors.js';

export function assertRequestIsOpen(status: RequestStatus): void {
  if (status !== 'open') {
    throw new DomainConflictError(
      'REQUEST_CLOSED',
      'This request is already closed.',
    );
  }
}

export function assertRequestCanBeReviewed(input: {
  status: RequestStatus;
  reviewState: RequestReviewState;
}): void {
  if (input.status !== 'open' || input.reviewState !== 'pending') {
    throw new DomainConflictError(
      'ALREADY_REVIEWED',
      'This request has already been reviewed.',
    );
  }
}

export function determineEditedState(
  originalContentMarkdown: string,
  editedContentMarkdown: string,
): boolean {
  return originalContentMarkdown !== editedContentMarkdown;
}

export function toReviewState(action: ReviewAction): RequestReviewState {
  switch (action) {
    case 'approve':
      return 'approved';
    case 'comment':
      return 'commented';
    case 'reject':
      return 'rejected';
  }
}

export function assertResumeRetryAllowed(status: ResumeStatus): void {
  if (status !== 'failed') {
    throw new DomainConflictError(
      'RESUME_RETRY_NOT_ALLOWED',
      'Only failed resume attempts can be retried.',
    );
  }
}
